...
function setNextOffset(offset) {
  nextOffset = offset ? offset.toString() : null;
}
...
export function fetchLogsTree(client, path, offset, resolver = null, _maxOffset = null) {
  ...
  if (!nextOffset && Number(offset) > maxOffset) {
    setNextOffset(Number(offset) - 25); // ensures commit data is fetched for newly added rows that need data from the previous request (requests are made in batches of 25).
